...
| RelayedFeed (USDe/USD via CCIP) | `0xC7bACd9Cf95dC4Fc1A4D950B2D4eDCbC7D91D07A` |
| Relayed market — "USDe < $0.95 for 1h (live Chainlink via CCIP)" | `0xe4B514eb25d2F989CD9896960Bc0338B34Fab0B4` |
...
| DepegSentinel (explicit 300k extraArgs gas limit) | `0xA795Baa9E4300EA015B094D14E152f123Edc94ad` |

## CCIP wiring

- Lane: Sepolia (selector `16015286601757825753`) → Arc (selector `3034092155422581607`), OnRamp v1.6.0.
- `RelayedFeed.setSource` is wired to the Sentinel. The sentinel sends an explicit `EVMExtraArgsV2` 300k destination gas limit (an earlier deploy with empty extraArgs under-provisioned gas and never executed). First price ($1.00) relayed; once delivered, `RelayedFeed.latestRound()` increments and the relayed market settles permissionlessly off real cross-chain Chainlink data.
